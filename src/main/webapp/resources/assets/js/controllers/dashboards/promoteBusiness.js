(function() {
    function PromoteBusinessCtrl($http, DTOptionsBuilder) {
        var self = this;
        ...
        self.loadBusinessObjects = function() {
            $http.get("loadBusiness").then(function(response) {
                ...
                angular.forEach(response.data.result, function(key, value) {
                    if (value != "null") self.businessObjects.push({
                        email: value,
                        count: key
                    });
                });
        ...
        self.promotionId = "";
        self.openPromotion = function(object) {
            self.promoteTitle = "";
            self.promoteDesc = "";
            $("#daterange-4").text(moment().format("MMMM D, YYYY"), moment().add(7, "days").format("MMMM D, YYYY"));
            if (object != undefined && object != null) {
                self.promoteTitle = object.promotionTitle;
                self.promoteDesc = object.promotionDetail;
                self.promoteId = object.promotionId;
        ...
        self.resetCounter = function() {
            ...
            $http.get("resetPromotionCounter").then(function(response) {
                self.promotionRequestCounter = 0;
            });
        };
        self.promotionRequestCounter = 0;
        self.promotionRequestMade = null;
        $http.get("getPromotionCounter").then(function(response) {
        ...
        self.loadPromotions = function() {
            self.promotions = [];
            $http.get("loadPromotions").then(function(response) {
                self.promotions = response.data.result.result;
            });
        };
        self.dtOptions = DTOptionsBuilder.newOptions().withDisplayLength(10).withOption("bLengthChange", true);
        self.loadBusinessObjects();
        self.loadPromotions();
    }
    angular.module("pixeladmin").controller("PromoteBusinessCtrl", PromoteBusinessCtrl);
})();