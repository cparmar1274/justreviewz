(function() {
    function CompetitorCtrl($http) {
        ...
        self.addCompetitor = function() {
            ...
            $http.post("addCompetitor", params).then(function(response) {
                if (response.data.success) {
                    self.message = true;
                    self.messageText = "Successfully added " + self.businessName + " as new competitor.";
                    self.getCompetitor();
        ...
        self.getCompetitor = function() {
            $http.get("getCompetitors").then(function(response) {
                if (response.data.success == true) self.competitors = response.data.data;
            });
        };
    ...
    angular.module("pixeladmin").controller("CompetitorCtrl", [ "$http", CompetitorCtrl ]);
})();