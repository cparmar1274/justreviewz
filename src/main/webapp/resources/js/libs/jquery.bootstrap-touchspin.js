        ...
        if ("destroy" !== o) {
            ...
            return this.each(function() {
                ...
                function d(o) {
                    ...
                    var n, s, e = B.prev(), a = B.next(), i = '<span class="input-group-addon bootstrap-touchspin-prefix">' + F.prefix + "</span>", p = '<span class="input-group-addon bootstrap-touchspin-postfix">' + F.postfix + "</span>";
                    e.hasClass("input-group-btn") ? (n = '<button class="' + F.buttondown_class + ' bootstrap-touchspin-down" type="button">' + F.buttondown_txt + "</button>", 
                    e.append(n)) : (n = '<span class="input-group-btn"><button class="' + F.buttondown_class + ' bootstrap-touchspin-down" type="button">' + F.buttondown_txt + "</button></span>", 
                    t(n).insertBefore(B)), a.hasClass("input-group-btn") ? (s = '<button class="' + F.buttonup_class + ' bootstrap-touchspin-up" type="button">' + F.buttonup_txt + "</button>", 
                    a.prepend(s)) : (s = '<span class="input-group-btn"><button class="' + F.buttonup_class + ' bootstrap-touchspin-up" type="button">' + F.buttonup_txt + "</button></span>", 
                    t(s).insertAfter(B)), t(i).insertBefore(B), t(p).insertAfter(B), M = o;
                }
                function f() {
                    var o;
                ...
                function w() {
                    if (F.booster) {
                        ...
                        return F.maxboostedstep && t > F.maxboostedstep && (t = F.maxboostedstep, P = Math.round(P / t) * t), 
                        Math.max(F.step, t);
                    }
                    return F.step;
                }
                function y() {
                    ...
                    var t = P, o = w();
                    (P += o) > F.max && (P = F.max, B.trigger("touchspin.on.max"), k()), N.input.val(Number(P).toFixed(F.decimals)), 
                    t !== P && B.trigger("change");
                }
                function _() {
                    g(), P = parseFloat(N.input.val()), isNaN(P) && (P = 0);
                    var t = P, o = w();
                    (P -= o) < F.min && (P = F.min, B.trigger("touchspin.on.min"), k()), N.input.val(P.toFixed(F.decimals)), 
                    t !== P && B.trigger("change");
                }
                function C() {
                    k(), z = 0, A = "down", B.trigger("touchspin.on.startspin"), B.trigger("touchspin.on.startdownspin"), 
                ...
                function D() {
                    k(), z = 0, A = "up", B.trigger("touchspin.on.startspin"), B.trigger("touchspin.on.startupspin"), 
